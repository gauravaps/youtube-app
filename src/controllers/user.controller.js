  ...
  const channel=await user.aggregate([
...

   {
      $addFields:{
         ...
         isSubscribed:{
            $cond:{if:{$in:[req.user1?._id,'$subscribers.subscriber']},
            then:true,
            else:false
         }
         }
...
const watchHistory=asynchandler(async(req,res)=>{
   const user1=await user.aggregate([{
      $match:{
         _id:new mongoose.Types.ObjectId(req.user1._id) 
      }
   },
   {$lookup:{
      from:'videos',
...
export{registerUser,loginUser,logoutUser,refreshAccessToken,changeCurrentPasswors,
 getCurentuser,updateAccount,updatAvatar,updateCoverimage,getUserChannelProfile,watchHistory} 